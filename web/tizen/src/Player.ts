...
// Tizen app id, used by Bitmovin's tweaks.app_id when the player is bundled
// inside the TV app (see config.xml `tizen:application id`).
const TIZEN_APP_ID = 'SMDBPKWOS1.SimidDemoTizen'
...
export default class Player {
  ...
  private registerPlayerModules() {
    ...
    // See: https://developer.bitmovin.com/playback/docs/getting-started-with-the-web-player-on-samsung-tizen
    ...
    Player.addModule(w.bitmovin.player.tizen.default)
  }
  ...
  private loadPlayer() {
    ...
    const playerConfig: any = {
      ...
      // No custom UI chrome on Tizen — keeps the modular player footprint small.
      ui: false,
      playback: {
        muted: true,
        autoplay: false
  ...
  private openPage(uri: string) {
    // No click-through / external navigation support on Tizen (remote-control only, no pointer).
    console.log('[Player] Open page requested (ignored on Tizen):', uri)
  }
