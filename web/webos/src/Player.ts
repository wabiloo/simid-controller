...
// webOS app id, used by Bitmovin's PlayerConfigBuilder.optimizeForPlatform when
// the player is bundled inside the TV app (see appinfo.json `id`).
const WEBOS_APP_ID = 'tv.broadpeak.simid.webos'
...
export default class Player {
  ...
  private registerPlayerModules() {
    // The modular Bitmovin build (see index.html) loads each module as a separate script,
    // exposing it under window.bitmovin.player.<moduleName>. Every loaded module must be
    // explicitly registered with the core Player class before instantiation.
    // See: https://developer.bitmovin.com/playback/docs/getting-started-with-the-web-player-on-lg-webos
    const w = window as any
    const Player = bitmovin.player.core.Player
    Player.addModule(w.bitmovin.player.polyfill.default)
    Player.addModule(w.bitmovin.player['engine-bitmovin'].default)
    Player.addModule(w.bitmovin.player['container-mp4'].default)
    Player.addModule(w.bitmovin.player['container-ts'].default)
    Player.addModule(w.bitmovin.player.mserenderer.default)
    Player.addModule(w.bitmovin.player.abr.default)
    Player.addModule(w.bitmovin.player.xml.default)
    Player.addModule(w.bitmovin.player.dash.default)
    Player.addModule(w.bitmovin.player.hls.default)
    Player.addModule(w.bitmovin.player.crypto.default)
    Player.addModule(w.bitmovin.player.webos.default)
  }
  ...
  private loadPlayer() {
    ...
    const playerConfig: any = {
      ...
      // No custom UI chrome on webOS — keeps the modular player footprint small.
      ui: false,
      playback: {
        muted: true,
        autoplay: false
      ...
      tweaks: {
        // Required by Bitmovin when the app HTML is bundled inside the TV app
        // package (rather than served from a remote host).
        app_id: WEBOS_APP_ID,
        file_protocol: true,
    ...
    // Reduce the in-memory buffer to lighten the load on resource-constrained TVs.
    if (bitmovin.player.core?.BufferType && bitmovin.player.core?.MediaType) {
      const bufferLevels: any = {}
      ...
      playerConfig.buffer = {
        [bitmovin.player.core.MediaType.Video]: bufferLevels,
        [bitmovin.player.core.MediaType.Audio]: bufferLevels
      }
    }

    this.player = new bitmovin.player.core.Player(this.playerElement, playerConfig)

    // React to pause events to show pause ads
    this.player.on(bitmovin.player.core.PlayerEvent.Paused, () => this.onVideoPaused())
    // React to play events to hide pause ads
    this.player.on(bitmovin.player.core.PlayerEvent.Playing, () => this.onVideoPlay())
  }
  ...
  private openPage(uri: string) {
    // No click-through / external navigation support on webOS (remote-control only, no pointer).
    console.log('[Player] Open page requested (ignored on webOS):', uri)
  }
